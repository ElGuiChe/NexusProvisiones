...
export default function ItemListContainer() {
  ...
  async function createProvisiones(provision) {
    const docRef = await addDoc(collection(db, "provisiones"), {
      ...
      centro_de_costo: provision.Centro_de_Costo,
      cuenta_asociada: provision.Cuenta_Asociada,
      codigo_articulo: provision.Código_Artículo,
      descripcion_del_servicio: provision.Descripción_del_Servicio,
      monto_equivalente_CLP: provision.Monto_equivalente_CLP,
      numero_de_contrato: provision.Número_de_Contrato,
      proveedor: provision.Proveedor,
      rut_proveedor: provision.RUT_Proveedor,
      uf: provision.UF,
      usd: provision.USD,
      vigencia_del_contrato: provision.Vigencia_del_Contrato,
      status: "En revisión",
      fecha_de_solicitud: new Date(),
    });
